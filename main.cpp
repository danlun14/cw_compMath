...
{
    return pow(D2, 3) - 9 * sin(x) * D2 - sin(x) + 9 * exp(x) * D1 + y / (pow(x + 2, 3));
}
...
double *f(double x, double *y)
{
    ...
    {
        c = (a + b) / 2;
        fa = diff(x, y[0], y[1], a) * diff(x, y[0], y[1], c);
        fb = diff(x, y[0], y[1], c) * diff(x, y[0], y[1], b);
...
double MethodShooting(double x0, double x1, double y0, double y1, double h)
{
    ...
    do
    {
        ...
        if (prev == fabs(y1 - (tmp1[0] + tmp2[0] + tmp4[0]) / 3))
        {
            return ((al + bt) / 2);
        }
        prev = fabs(y1 - (tmp1[0] + tmp2[0] + tmp4[0]) / 3);
...
vector<double> calcM(vector<double> a, vector<double> b,
...
double splineCalc(int i, double *M, pair<double, double> *v, double x)
{
...
double cubicSpline(double x, int n, pair<double, double> *v)
{
    int i = 0;
    double s = 0;
    vector<double> M(n - 2);
...
int main()
{
    ...
    {
        double tmp = cubicSpline(i, v.size(), v.data());
